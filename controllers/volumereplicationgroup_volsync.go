...
func (v *VRGInstance) reconcileVolSyncAsPrimary() bool {
	...
	for _, pvc := range v.volSyncPVCs {
		...
		if err != nil {
			...
			setVolSyncProtectedPVCConditionError(&protectedPVC.Conditions, v.instance.Generation,
				"VolSync setup failed")

			return false
		}

		setVolSyncProtectedPVCConditionReady(&protectedPVC.Conditions, v.instance.Generation,
			"VolSync setup succeeded")
